...
fftm.services <- ff.prepareWordcloud(text = ffdf.services)
ff.plotWordcloud(terms = fftm.services, filename = "FF_service_cloud.png")

# description cloud 
fftm.descriptions <- ff.prepareWordcloud(text = ffdf$state.description)
fftm.descriptions <- fftm.descriptions[, setdiff(names(fftm.descriptions), 
                                                 c("freifunk", "freifnunk", "freifunker_innen", "freifunkerinnen"))]
ff.plotWordcloud(terms = fftm.descriptions, filename = "FF_description_cloud.png")
